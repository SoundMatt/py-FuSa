...
_OBJECTIVES = [
    ("62443-1",  "2-1 §4.3",    "Security management policy",                   "SL-1", ".fusa-iec62443.json"),
    ("62443-2",  "2-3 §4.3",    "Patch and vulnerability management",            "SL-1", "vuln.json"),
    ("62443-3",  "3-2 §9.4",    "Cybersecurity risk assessment (TARA)",          "SL-1", "tara.json"),
    ("62443-4",  "3-3 SR1",     "System security requirements",                  "SL-2", "check-report.json"),
    ("62443-5",  "4-1 §8.1",    "Secure development lifecycle (SDL policy)",     "SL-1", ".fusa.json"),
    ("62443-6",  "4-1 §8.2",    "Security requirements specification",           "SL-1", ".fusa-reqs.json"),
    ("62443-7",  "4-1 §8.4",    "Secure design (component boundary diagram)",   "SL-2", "boundary.json"),
    ("62443-8",  "4-2 CR6.2",   "Security policy document (SECURITY.md)",        "SL-1", "SECURITY.md"),
    ("62443-9",  "4-2 CR6.2.1", "Cyber incident response plan",                 "SL-2", "INCIDENT-RESPONSE.md"),
    ("62443-10", "4-1 §8.6",    "Supply-chain integrity (SBOM)",                "SL-2", "sbom.json"),
    ("62443-11", "4-1 §8.8",    "Tool qualification evidence",                  "SL-3", "qualify-report.json"),
...
def run(project_root: str, cfg: Config, sl: str = "SL-2") -> dict:
    ...
    for obj_id, clause, title, sl_min, evidence_file in _OBJECTIVES:
        ...
        obj = {"id": obj_id, "clause": clause, "title": title,
               "slMin": sl_min, "status": status, "evidence": evidence}
        if status == "gap":
    ...
    return {
        ...
        "project": module, "standard": "iec62443", "sl": sl,
...
def render_text(doc: dict) -> str:
    ...
    for obj in doc["objectives"]:
        ...
        lines.append(f"  {marker} {obj['id']:10s} {obj['clause']:15s} {obj['status']:10s} {obj['title']}")
    return "\n".join(lines)
