...
def test_qualify_all_pass():
    report = qualify.run()
    assert report.failed == 0, f"qualification failures: {[r.name for r in report.results if r.result != 'PASS']}"
...
def test_qualify_result_values():
    report = qualify.run()
    valid = {qualify.RESULT_PASS, qualify.RESULT_FAIL, qualify.RESULT_SKIP, qualify.RESULT_ERROR}
