...
def test_hlr_llr_valid_hierarchy_no_violations():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        _write_reqs(tmpdir, [
            ...
            {"id": "REQ-LLR1", "title": "LLR 1", "level": "LLR", "parent_id": "REQ-HLR1"},
        ])
        cfg = default()
        matrix = trace.build(tmpdir, cfg)
        assert matrix.hlr_violations == []
...
def test_hlr_llr_orphan_llr_unknown_parent():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        _write_reqs(tmpdir, [
            {"id": "REQ-HLR1", "title": "HLR 1", "level": "HLR"},
            {"id": "REQ-LLR1", "title": "LLR", "level": "LLR", "parent_id": "REQ-DOESNOTEXIST"},
        ])
        cfg = default()
        matrix = trace.build(tmpdir, cfg)
        kinds = [v.kind for v in matrix.hlr_violations]
...
def test_hlr_llr_text_shows_hierarchy():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        _write_reqs(tmpdir, [
            ...
            {"id": "REQ-LLR1", "title": "Low level", "level": "LLR", "parent_id": "REQ-HLR1"},
        ])
        cfg = default()
        matrix = trace.build(tmpdir, cfg)
...
def test_trace_strict_hlr_llr_flag_exits_1():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        out = io.StringIO()
        err = io.StringIO()
        code = run(["trace", "--dir", tmpdir, "--strict-hlr-llr"], stdout=out, stderr=err)
...
def test_qualify_cli_qualification_flags_json():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        out = io.StringIO()
        code = run([
            "qualify", "--dir", tmpdir, "--format", "json",
            "--qualification-method", "independent",
            "--qualifier", "TestOrg",
            "--record-uri", "https://dossier.example/q1",
        ], stdout=out)
        assert code == pyfusa.EXIT_OK
...
def test_mcdc_all_conditions_covered():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        _write_llvm_coverage(cov_path, {
            "data": [{
                "functions": [{
                    "name": "my_func",
                    "mcdc_records": [{"conditions": [
                        {"covered_true_count": 5, "covered_false_count": 3},
        ...
        _write_llvm_coverage(cov_path, {
            "data": [{
                "functions": [{
                    "name": "my_func",
                    "mcdc_records": [{"conditions": [
                        {"covered_true_count": 5, "covered_false_count": 0},  # not covered
                    ]}],
                }]
            }]
        })
...
def test_mcdc_missing_file_returns_error():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        cfg = default()
        doc = coverage.run(tmpdir, cfg, mcdc=True,
                           mcdc_file=os.path.join(tmpdir, "nonexistent.json"))
        assert "mcdc" in doc
...
def test_mcdc_json_output_structure():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        doc = coverage.run(tmpdir, cfg, mcdc=True, mcdc_file=cov_path)
        mc = doc["mcdc"]
        for key in ("totalConditions", "coveredConditions", "coveragePct",
                    "threshold", "passed", "uncoveredFunctions", "functions"):
...
def test_mcdc_threshold_applied():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        doc_strict = coverage.run(tmpdir, cfg, mcdc=True, mcdc_file=cov_path, mcdc_threshold=100.0)
        assert doc_strict["mcdc"]["passed"] is False
        # At 40% threshold, 1/2 = 50% passes
        doc_lax = coverage.run(tmpdir, cfg, mcdc=True, mcdc_file=cov_path, mcdc_threshold=40.0)
...
def test_mcdc_cli_flag_json_output():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        out = io.StringIO()
        code = run([
            "coverage", "--dir", tmpdir, "--format", "json",
            "--mcdc", "--mcdc-file", cov_path,
        ], stdout=out)
        doc = json.loads(out.getvalue())
        assert "mcdc" in doc
        assert doc["mcdc"]["passed"] is True
